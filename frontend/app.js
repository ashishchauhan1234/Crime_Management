...
function getUserByUsername(username) {
    return readUsers().find(x => x.username === username);
}

// FIRs
function readFIRs() {
    return read(KEY_FIRS);
}

function writeFIRs(list) {
    write(KEY_FIRS, list);
}

// Criminals
function readCriminals() {
    return read(KEY_CRIM);
}

function writeCriminals(list) {
    write(KEY_CRIM, list);
}

// Session
function currentUser() {
    return JSON.parse(sessionStorage.getItem('cims_user') || 'null');
}

function loginSession(user) {
    sessionStorage.setItem('cims_user', JSON.stringify(user));
}
...
function deleteFIR(id) {
    ...
    const list = readFIRs().filter(x => x.id !== id);
    writeFIRs(list);
    renderFIRTable();
}

// --- Criminals page ---
function criminalsInit() {
    if (!requireLogin()) return;
    renderCrimTable();
}
...
function submitCrim(form) {
    ...
    const list = readCriminals();
    list.push(c);
    writeCriminals(list);
    alert('Criminal saved');
    form.reset();
    renderCrimTable();
}

function renderCrimTable() {
    const tbody = $('#crimTable tbody');
    const list = readCriminals().slice().reverse();
    tbody.innerHTML = '';
    list.forEach(c => {
        const tr = document.createElement('tr');
...
function viewCrim(id) {
    const c = readCriminals().find(x => x.id === id);
    alert(`${c.name} — ${c.crime}\nAlias:${c.alias}\nAddr:${c.address}\nNotes:${c.notes}`);
}

function deleteCrim(id) {
    if (!confirm('Delete record?')) return;
    writeCriminals(readCriminals().filter(x => x.id !== id));
    renderCrimTable();
}
...
function doSearch() {
    ...
    if (firs.length) {
        ...
        firs.forEach(f => {
            ...
            d.className = 'card';
            d.innerHTML = `<b>${f.id}</b> — ${f.complainant} — ${f.type}`;
            out.appendChild(d);
        });
    }
    if (crim.length) {
        const h = document.createElement('h4');
        h.textContent = 'Criminals';
        out.appendChild(h);
        crim.forEach(c => {
            const d = document.createElement('div');
            d.className = 'card';
            d.innerHTML = `<b>${c.name}</b> — ${c.crime}`;
            out.appendChild(d);
        });
    }
    ...
    const counts = {};
    readFIRs().forEach(f => counts[f.type] = (counts[f.type] || 0) + 1);
    let rpt = '<h4>Report: FIRs by Type</h4><ul>';
    Object.entries(counts).forEach(([k, v]) => rpt += `<li>${k}: ${v}</li>`);
    rpt += '</ul>';
